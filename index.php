						...
						<tbody>
							...
							if(mysqli_num_rows($result) == 0) {
								...
								while($row = mysqli_fetch_array($result)) {
									echo "<tr>".($info["skulls"] == true ? "<td class=\"text-center\"><img src=\"https://crafatar.com/renders/head/".$row["uuid"]."?scale=2&default=MHF_Steve&overlay\" alt=\"".$row["name"]."\"></td>" : "")."<td>".(strpos($row["name"], ".") === false ? "<a href=\"user/?user=".$row["name"]."\">" : "").$row["name"].(strpos($row["name"], ".") === false ? "</a>" : "")."</td><td>".$row["reason"]."</td>".($info["skulls"] == true ? "<td class=\"text-center\"><img src=\"https://crafatar.com/renders/head/".json_decode(file_get_contents("https://www.theartex.net/cloud/api/minecraft/?sec=uuid&username=".$row["operator"]),true)["data"]["uuid"]."?scale=2&default=MHF_Steve&overlay\" alt=\"".$row["operator"]."\"></td>" : "")."<td>".$row["operator"]."</td><td>".$date->local($row["start"] / 1000, "F jS, Y")."<br><span class=\"badge\">".$date->local($row["start"] / 1000, "g:i A")."</span></td><td>".($row["end"] == "-1" ? $lang["error_not_evaluated"] : $date->local($row["end"] / 1000, "F jS, Y")."<br><span class=\"badge\">".$date->local($row["end"] / 1000, "g:i A")."</span>")."</td><td>".$lang[strtolower($row["punishmentType"])]."</td><td>".(in_array($row["punishmentType"], array("BAN", "TEMP_BAN", "MUTE", "TEMP_MUTE", "IP_BAN", "WARNING", "TEMP_WARNING")) ? (mysqli_num_rows(mysqli_query($con, "SELECT * FROM `".$info["table"]."` WHERE punishmentType='".$row["punishmentType"]."' AND start='".$row["start"]."'")) > 0 && ($row["end"] == "-1" || $date->local(round(microtime(true) * 1000) / 1000, "U") < $date->local($row["end"] / 1000, "U")) ? $lang["active"] : $lang["inactive"]) : $lang["error_not_evaluated"])."</td></tr>";
								}
							}
							?>
						</tbody>
